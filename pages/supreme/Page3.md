...
- 将生物科技生成器的生产速率提高数倍。具体的倍数为 $\dfrac {n}{64}$（$n$ 为组件数目数量），**四舍五入**到整数。
...
按照一定比例降低生物科技生成器的功率。具体的百分比为 $\left [\dfrac {10}{64}\times n(1+k)\right ]\%$
...
$2000\rm \; J/tick\times (1-20\% )\times (1-20\%)\times (1-50\%)\times (1-50\%)=320\; J/tick$。
...
- 每次完成生产时，额外产出 knT 个物品。

  - 其中，T 为组件的等级，n 为物品数目。如果 $n\times T>q$（$q$ 为没有组件时，生物科技生成器每次的产出），会改为额外产出 $kq$ 个物品。
